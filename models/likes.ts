...
export const like = async  (id:any, uid:any) =>{
 ...
 let query = `INSERT INTO petslikes (petid,userid) VALUES (${id},${uid}) ON CONFLICT ON CONSTRAINT  NoDuplicateLike DO NOTHING RETURNING userid;`   
...
export const dislike = async  (id:any, uid:any)=> {
  let query = "DELETE FROM petslikes WHERE petid=? AND userid=?; ";
   try{
...
//count the likes for an pet

export const count = async  (id:any) =>{
  let query = "SELECT count(1) as likes FROM petslikes WHERE petid=?;";
