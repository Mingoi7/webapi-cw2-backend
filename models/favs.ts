...
//let query1 = `SELECT * FROM favs WHERE  petid=${id} AND userid=${uid} `
 let query = `INSERT INTO favs (petid,userid) VALUES (${id},${uid}) ON CONFLICT ON CONSTRAINT  NoDuplicateFav DO NOTHING RETURNING userid;`   
 try{
...
export const removeFav = async (id:any, uid:any) =>{
   let query = `DELETE FROM favs WHERE petid=${id} AND userid=${uid} ;`;
   try{
