...
export const getMsg= async  (id:any)=> {
  let query = "SELECT * FROM msgs WHERE petid=?;";
...
export const add_Msg = async (id:any, uid:any,uname:any,msg:any) =>{
 console.log('body query ', msg)
  ...
  console.log ("msgtxt from query ",msgtxt)
    let query = `INSERT INTO msgs (petid,userid,username,messagetxt) VALUES (${id},${uid},'${uname}','${msgtxt}') `  
  try{
...
let query = "DELETE FROM msgs WHERE petid=? AND messagetxt=?; ";
   try{
